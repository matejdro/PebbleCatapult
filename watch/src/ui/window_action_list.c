...
{
    WindowActionList* window_action_list = context;

    if (window_action_list->directory_id == bucket_metadata.id)
    {
        load_menu(context);
    }
}
...
static void window_hide(Window* window)
{
    ...
    bucket_sync_clear_bucket_data_change_callback(bucket_update_callback, (void*)window_action_list);
}
...
void window_action_list_show(uint8_t directory_id)
{
    ...
    window_action_list->directory_id = directory_id;
...
static void load_menu(WindowActionList* window)
{
    ...
    if (bucket_sync_load_bucket(window->directory_id, tmp))
    {
        const uint8_t count = tmp[0];
        if (count == 0 && window->directory_id == 1)
        {
